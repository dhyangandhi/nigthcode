...
const app = new Hono().get("/callback", (c) => {
    ...
    try {
        ...
        const redireUrl = `http://localhost:${port}/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`;

        return c.redirect(redireUrl);

    } catch {
        return c.text("Invalid authentioncation state", 400);
    }
});
