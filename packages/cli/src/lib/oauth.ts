...
export async function preformLogin() {
    ...
    return new Promise<{ token: string }>((resolve, reject) => {
        const server = Bun.serve({
            ...
            async fetch(req) {
                ...
                if (error) {
                    ...
                    return new Response(`Authentiction failed: ${msg}`, { status: 400});
                }
                ...
                if (!code || !state) {
                    settled = true;
                    reject(new Error("Messing code or state"));
                    setTimeout(() => server.stop(), 500);
                ...
                try {
                    ...
                    if (!tokenRes.ok) {
                        ...
                        throw new Error(details || "Failed to exchange authoriztion code");
                    }
                    ...
                    setTimeout(() => server.stop(), 500);
                    return new Response(`Authtication failed: ${message}`, { status: 400 });
                }
            },
        });
        ...
        if (typeof port != "number") {
            server.stop();
            reject(new Error("Falied to start callback server"));
            return;
        }
